  ...
  async (req, res) => {
    try {
      ...
      const docs = files.map((file) => {
        ...
        const url = path
          .join(type === "image" ? "images/gallery" : "videos", file.filename)
          .replace(/\\/g, "/");
        return {
          name: file.originalname, // store original filename
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      const updateData = {};
      if (title) updateData.name = title; // update name instead of title

      if (req.file) {
        const type = req.file.mimetype.startsWith("image") ? "image" : "video";
        const url = path
          .join(type === "image" ? "images/gallery" : "videos", req.file.filename)
          .replace(/\\/g, "/");
