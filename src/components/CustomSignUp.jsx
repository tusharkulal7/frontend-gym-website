...
const CustomSignUp = () => {
  ...
  const handleGoogleSignUp = async () => {
    if (!isLoaded || !signUp) {
      setError('Sign up service is not ready yet. Please try again shortly.');
      return;
    }
    ...
    try {
      ...
      const redirectUrlComplete = `${window.location.origin}/verification`;
      ...
      <div className="w-full max-w-sm sm:max-w-md lg:max-w-lg xl:max-w-xl mx-auto">
        ...
        <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-xl sm:rounded-2xl shadow-2xl px-4 py-6 sm:px-6 lg:px-8 lg:py-10">
          ...
          <p className="text-gray-300 text-sm sm:text-base text-center mb-6">
            We support passwordless sign ups through your Google account for maximum security.
          </p>
          <button
            type="button"
          ...
          <p className="text-gray-400 text-xs sm:text-sm text-center mt-4">
            Already have an account? <a href="/login" className="text-red-400 hover:text-red-300">Sign in with Google</a>
          </p>
        </div>
      </div>
      {/* Decorative Elements */}
